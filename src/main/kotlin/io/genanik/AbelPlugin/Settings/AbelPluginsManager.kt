package io.genanik.AbelPlugin.Settings

import net.mamoe.mirai.message.data.MessageChain
import net.mamoe.mirai.utils.MiraiLogger

open class AbelPluginsManager(newLogger: MiraiLogger) {
    ...
    val argsMap = mutableMapOf<String, (Long) -> MessageChain >()
    val commandHelpInf = mutableMapOf<String, String>()
    ...
    val functionMap = mutableMapOf<String, MutableList<Long>>() //群号存在这里面就是关闭了
    val functionHelpInf = mutableMapOf<String, String>()
...
class UserPluginManager(newLogger: MiraiLogger): AbelPluginsManager(newLogger)

class AdminPluginsManager(newLogger: MiraiLogger, newTargetUsers: ArrayList<Long>):
    AbelPluginsManager(newLogger) {

    private var adminList = newTargetUsers

    /**
     * 判断用户是否为Admin
     */
    fun isAdmin(checkID: Long): Boolean {
        return adminList!!.contains(checkID)
    }
